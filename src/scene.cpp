...
void Scene::loadSceneBox(Config *config)
{
    ...
    {
        case DIFFUSE:
            tallBoxMaterial = whiteDiffuseMaterial;
            break;
        case MIRROR:
            tallBoxMaterial = make_shared<MirrorMaterial>(vec3(0.725, 0.71, 0.68));
            break;
        case MICROFACET:
            tallBoxMaterial = make_shared<MicrofacetMaterial>(vec3(0.725, 0.71, 0.68), config->roughness);
            break;
        default:
...
void Scene::loadSceneBunny(Config *config)
{
    ...
    {
        case DIFFUSE:
            bunnyMaterial = whiteDiffuseMaterial;
            break;
        case MIRROR:
            bunnyMaterial = make_shared<MirrorMaterial>(vec3(0.725, 0.71, 0.68));
            break;
        case MICROFACET:
            bunnyMaterial = make_shared<MicrofacetMaterial>(vec3(0.725, 0.71, 0.68), config->roughness);
            break;
        default:
            throw std::runtime_error("Material not supported.");
    }
...
vec3 Scene::shadeDiffuse(const HitRecord &hitRecord, const Ray &rayIn)
{
    ...
    if (glm::linearRand(0.0f, 1.0f) < RUSSIAN_ROULETTE)
    {
        ...
        if (reflectorStatus && !reflectorHitRecord.material->hasEmission())
        {
            ...
            lightInDir += shade(reflectorHitRecord, Ray(hitRecord.point, rayOutDir)) * brdf * cosTheta / pdf /
        ...
        if (NoL > 0.0f)
        {
            ...
            if (reflectorStatus && !reflectorHitRecord.material->hasEmission())
            {
                ...
                if (NoL > 0.0f)
                {
                    ...
                    lightInDir += shade(reflectorHitRecord, Ray(hitRecord.point, L)) * F * G * VoH / (NoH * NoV) /
                                  (float) RUSSIAN_ROULETTE;
                }
            }
        }
...
inline vec3 Scene::shade(const HitRecord &hitRecord, const Ray &rayIn)
{
    ...
    {
        case DIFFUSE:
            return shadeDiffuse(hitRecord, rayIn);
        case MIRROR:
            return shadeMirror(hitRecord, rayIn);
        case MICROFACET:
            return shadeMicrofacet(hitRecord, rayIn);
        default:
            throw std::runtime_error("Material not supported.");
    }
}
