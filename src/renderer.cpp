        ...
        std::thread t([this, threadIndex, blockLength]()
                      {
                          ...
                          int end = (threadIndex == config->threadCount) ? SCREEN_HEIGHT : (threadIndex + 1) *
                                                                                           blockLength;
    ...
    for (auto &sampleFramebuffer: sampleFramebufferPool)
    {
        for (auto &pixel: sampleFramebuffer)
        {
            framebuffer[pixelIndex] =
                    (float) currentSampleCount / ((float) currentSampleCount + 1.0f) * framebuffer[pixelIndex] +
...
void Renderer::buildGBuffer()
{
    for (int i = 0; i < SCREEN_HEIGHT; ++i)
    {
        for (int j = 0; j < SCREEN_WIDTH; ++j)
        {
            ...
            {  // set background color for GBuffer
...
void Renderer::filterByBilateral()
{
    ...
    for (int i = 0; i < SCREEN_HEIGHT; ++i)
    {
        for (int j = 0; j < SCREEN_WIDTH; ++j)
        {
            ...
            for (int col = j - halfSize; col <= j + halfSize; ++col)
            {
                ...
                float weight = bilateralFilter(col - j, colorDistance, BILATERAL_FILTER_DISTANCE_SIGMA,
                                               BILATERAL_FILTER_COLOR_SIGMA);
    ...
    for (int i = 0; i < SCREEN_HEIGHT; ++i)
    {
        for (int j = 0; j < SCREEN_WIDTH; ++j)
        {
            ...
            for (int row = i - halfSize; row <= i + halfSize; ++row)
            {
                vec3 colorDistance = framebufferAfterHorizontal[row * SCREEN_WIDTH + j] -
                                     framebufferAfterHorizontal[i * SCREEN_WIDTH + j];
                float weight = bilateralFilter(row - i, colorDistance, BILATERAL_FILTER_DISTANCE_SIGMA,
                                               BILATERAL_FILTER_COLOR_SIGMA);
...
void Renderer::filterByJoint()
{
    ...
    for (int i = 0; i < SCREEN_HEIGHT; ++i)
    {
        for (int j = 0; j < SCREEN_WIDTH; ++j)
        {
            ...
            vec3 pixel = glm::clamp(framebuffer[i * SCREEN_WIDTH + j], averageColor - 1.0f * sigmaColor,
                                    averageColor + 1.0f * sigmaColor);
        ...
        for (int j = 0; j < SCREEN_WIDTH; ++j)
        {
            ...
            for (int row = i - halfSize; row <= i + halfSize; ++row)
            {
                for (int col = j - halfSize; col <= j + halfSize; ++col)
                {
                    ...
                    float gradientX =
                            gBufferNormal[i * SCREEN_WIDTH + j].x / (gBufferNormal[i * SCREEN_WIDTH + j].z + 0.0005f);
                    float distanceX =
                            gBufferPosition[i * SCREEN_WIDTH + j].x - gBufferPosition[row * SCREEN_WIDTH + col].x;
                    float predictX = gradientX * distanceX;
                    predictX = predictX > 0.0f ? predictX : -predictX;
                    float gradientY =
                            gBufferNormal[i * SCREEN_WIDTH + j].y / (gBufferNormal[i * SCREEN_WIDTH + j].z + 0.0005f);
                    float distanceY =
                            gBufferPosition[i * SCREEN_WIDTH + j].y - gBufferPosition[row * SCREEN_WIDTH + col].y;
                    float predictY = gradientY * distanceY;
                    predictY = predictY > 0.0f ? predictY : -predictY;
                    float weightDepth = powf(2.718281828f, -distanceZ /
                                                           ((float) JOINT_FILTER_DEPTH_SIGMA * (predictX + predictY) +
                                                            0.0005f));

                    // weight of normal
                    float baseNormal = std::max(0.0f, dot(gBufferNormal[i * SCREEN_WIDTH + j],
                                                          gBufferNormal[row * SCREEN_WIDTH + col]));
                    float weightNormal = powf(baseNormal, JOINT_FILTER_NORMAL_SIGMA);

                    // weight of color
                    float distanceColor = length(framebufferAfterRemoval[i * SCREEN_WIDTH + j] -
                                                 framebufferAfterRemoval[row * SCREEN_WIDTH + col]);
                    float weightColor = powf(2.718281828f, -distanceColor / ((float) JOINT_FILTER_COLOR_SIGMA *
                                                                             sqrtf(length(varianceColor)) + 0.0005f));
...
void Renderer::filterByOutlierRemoval()
{
    ...
    for (int i = 0; i < SCREEN_HEIGHT; ++i)
    {
        for (int j = 0; j < SCREEN_WIDTH; ++j)
        {
            ...
            vec3 pixel = glm::clamp(framebuffer[i * SCREEN_WIDTH + j], averageColor - k * sigmaColor,
                                    averageColor + k * sigmaColor);
            framebufferAfterFilter.emplace_back(pixel);
        }
    }
}
