...
{
    return 1.0f / (sqrtf(2.0f * (float) PI) * sigma) *
           powf(2.718281828f, -(float) (distance * distance) / (2.0f * sigma * sigma));
}

float bilateralFilter(int distance, vec3 colorDistance, float sigma, float colorSigma)
{
    return powf(2.718281828f, -(float) (distance * distance) / (2.0f * sigma * sigma) -
                              length(colorDistance) * length(colorDistance) / (2.0f * colorSigma * colorSigma));
}