...
TreeNode *BVHAccelStructure::buildBVHTree(vector<int> &triangleIndices)
{
    ...
    {  // lengthZ >= lengthX && lengthZ >= lengthY
        std::sort(triangleIndices.begin(), triangleIndices.end(), [this](int indexA, int indexB)
        {
...
TreeNode *SAHAccelStructure::findBestSibling(int leafIndex)
{
    ...
    std::priority_queue<TreeNode *, vector<TreeNode *>, std::function<bool(TreeNode *, TreeNode *)>> siblingQueue(
    ...
    while (!siblingQueue.empty())
    {
        ...
        {
            currentSibling->leftChild->cost =
                    areaBound(unionBound(currentSibling->leftChild->box, containerAABB[leafIndex])) + costAncestor;
            currentSibling->rightChild->cost =
                    areaBound(unionBound(currentSibling->rightChild->box, containerAABB[leafIndex])) + costAncestor;
