...
Application::Application() : nanogui::Screen(
        nanogui::Vector2i(SCREEN_WIDTH + FORM_WIDTH + PAD_WIDTH + 3 * MARGIN, APP_HEIGHT + 2 * MARGIN),
        "V-PathTracer", false)
{
    ...
    vMaterial->setTooltip(
            "If set to Diffuse, the Roughness will be ignored; if set to Microfacet, the Sample Way will be ignored.");
    ...
    std::thread threadRun([this]()
                          {
                              ...
                              if (this->config->gBuffer != GBUFFER_NONE)
                              {
                                  ...
                                  {
                                      case DEPTH:
                                          data = this->renderer->getGBufferDepth();
                                          break;
                                      case NORMAL:
                                          data = this->renderer->getGBufferNormal();
                                          break;
                                      case COLOR:
                                          data = this->renderer->getGBufferColor();
                                          break;
                                      case POSITION:
                                          data = this->renderer->getGBufferPosition();
                                          break;
                                      default:
                                          throw std::runtime_error("GBuffer type error.");
                                  }
                              ...
                              } else
                              {
                                  ...
                                  for (int i = 0; i < this->config->sampleCount; ++i)
                                  {
                                      ...
                                      if (this->config->filterType != FILTER_NONE &&
                                          i == this->config->sampleCount - 1)
                                          data = this->renderer->getFramebufferAfterFilter();
                                  ...
                                  if (this->config->filterType != FILTER_NONE)
                                      this->renderer->dumpFramebufferAfterFilter();
                                  cout << endl;
                              }
                              auto renderEndTime = std::chrono::high_resolution_clock::now();
                              auto renderDuration = std::chrono::duration_cast<std::chrono::milliseconds>(
